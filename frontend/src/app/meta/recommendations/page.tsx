...
export default function MetaRecommendationsPage() {
  const [items, setItems] = useState<MetaRec[]>([])
  ...
  const onApply = async (id: string) => {
    ...
    setBusyMode('apply')
    try {
      const r = await fetch(`${API_BASE}/api/meta/recommendations/${id}/apply`, {
        method: 'POST',
        credentials: 'include',
  ...
  const onDismiss = async (id: string, reason: string) => {
    ...
    setBusyMode('dismiss')
    try {
      const r = await fetch(`${API_BASE}/api/meta/recommendations/${id}/dismiss`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
